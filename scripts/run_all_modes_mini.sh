...
# 输入与输出
PROMPT_FILE="data/prompt_icml_sensitive_500.txt"  # 使用新的 500 条敏感 Prompt
BASE_OUT_DIR="results_sensitive_500"
LOG_DIR="logs_sensitive_500"

# 统一超参 (FLUX-dev) - 保持保守参数，靠 Prompt 数量和敏感度拉开差距
MODEL="flux-dev"
STEPS=50
WIDTH=1024
HEIGHT=1024
INTERVAL=6         # 保持 6
MAX_ORDER=2
FIRST_ENHANCE=10   # 保持 10
HICACHE_SCALE=0.7  # 保持 0.7

# 创建目录
mkdir -p "$BASE_OUT_DIR" "$LOG_DIR" "outputs"

echo "========================================================"
echo "🚀 开始 ICML Sensitive 实验 (500 敏感 Prompts, 保守参数)"
...
CUDA_VISIBLE_DEVICES=6 /usr/bin/time -v bash scripts/sample.sh \
    ...
    --prompt_file "$PROMPT_FILE" --limit 500 \
...
# ------------------------------------------------------
KERNEL_PATH="outputs/kernel_sensitive_i${INTERVAL}.pt"
echo "[7/8] Running EigenCache Calibration (Generating Kernel)..."
# 注意：校准使用前 30 条 prompt，足够覆盖分布
