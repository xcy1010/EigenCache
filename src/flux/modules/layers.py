...
from flux.modules.cache_functions import (
    force_init,
    cache_cutfresh,
    cache_cutfresh_with_ClusCa,
    update_cache,
    propagation_update_cache,
)
        ...
        if cache_dic is None:
            ...
            if (current["type"] == "full") or (current["type"] == "Delta-Cache"):
                ...
                if cache_dic["cache_type"] == "attention":
                    cache_dic["attn_map"][-1][current["stream"]][current["layer"]]["txt_mlp"] = cache_dic[
                        "attn_map"
                    ][-1][current["stream"]][current["layer"]]["total"][:, : txt.shape[1]]
                    cache_dic["attn_map"][-1][current["stream"]][current["layer"]]["img_mlp"] = cache_dic[
                        "attn_map"
                    ][-1][current["stream"]][current["layer"]]["total"][:, txt.shape[1] :]
                ...
                if (
                    ...
                    or cache_dic.get("enable_feature_collection", False)
                    or cache_dic.get("mode") == "ToCa"
                ):
                ...
                current["module"] = "img_mlp"
                fresh_indices, fresh_tokens_img = cache_cutfresh_with_ClusCa(
                    cache_dic=cache_dic, tokens=img, current=current
                )
                fresh_tokens_img = self.img_mlp(
                    (1 + img_mod2.scale) * self.img_norm2(fresh_tokens_img) + img_mod2.shift
                ...
                if cache_dic["propagation_ratio"] > 0.0:
                    propagation_update_cache(
                        fresh_indices, fresh_tokens=fresh_tokens_img, cache_dic=cache_dic, current=current
                    )
                else:
                    update_cache(
                        fresh_indices, fresh_tokens=fresh_tokens_img, cache_dic=cache_dic, current=current
                ...
                current["module"] = "txt_mlp"
                fresh_indices, fresh_tokens_txt = cache_cutfresh_with_ClusCa(
                    cache_dic=cache_dic, tokens=txt, current=current
                )
                fresh_tokens_txt = self.txt_mlp(
                    (1 + txt_mod2.scale) * self.txt_norm2(fresh_tokens_txt) + txt_mod2.shift
                ...
                update_cache(
                    fresh_indices, fresh_tokens=fresh_tokens_txt, cache_dic=cache_dic, current=current
                )
                ...
                img = (
                    img
                    + img_mod1.gate * cache_dic["cache"][-1]["double_stream"][current["layer"]]["img_attn"][0]
                )
                txt = (
                    txt
                    + txt_mod1.gate * cache_dic["cache"][-1]["double_stream"][current["layer"]]["txt_attn"][0]
                )
                img = (
                    img
                    + img_mod2.gate * cache_dic["cache"][-1]["double_stream"][current["layer"]]["img_mlp"][0]
                )
                txt = (
                    txt
                    + txt_mod2.gate * cache_dic["cache"][-1]["double_stream"][current["layer"]]["txt_mlp"][0]
                )
    ...
    def forward(self, x: Tensor, vec: Tensor, pe: Tensor, **kwargs) -> Tensor:
        ...
        if cache_dic is None:
            ...
            if current["type"] == "full":
                ...
                # 🔥 修复：也在特征收集时调用 derivative_approximation
                if (
                    cache_dic.get("taylor_cache", False)
                    or cache_dic.get("enable_feature_collection", False)
                    or cache_dic.get("mode") == "ToCa"
                ...
                current["module"] = "skipped"
                single_last_layer = cache_dic.get("_single_stream_last_layer", None)
                if single_last_layer is None:
                    single_last_layer = max(cache_dic["cache"][-1]["single_stream"].keys())
                    cache_dic["_single_stream_last_layer"] = single_last_layer
                if current["layer"] == single_last_layer:
