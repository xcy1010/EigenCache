def cache_init(
    ...
    :param timesteps: Timestep sequence
    :param model_kwargs: Model parameters (currently unused)
    :param mode: Cache mode ('original', 'ToCa', 'Taylor', 'HiCache', 'Delta', 'ClusCa')
    :param interval: Cache refresh interval (override default value)
    :param max_order: Maximum order (override default value)
    :param first_enhance: Initial enhancement steps (override default value)
    :param hicache_scale: HiCache polynomial scaling factor (override default value)
    """
    cache_dic = {}
    cache = {}
    cache_index = {}
    ...
    for j in range(38):
        # Single-stream blocks use a *global* layer index: [19, 56].
        # Keep all cache sub-structures aligned to `current["layer"]` from `flux/model.py`.
        layer_idx = j + 19

        cache[-1]["single_stream"][layer_idx] = {}
        cache_index[-1][layer_idx] = {}

        cache_dic["attn_map"][-1]["single_stream"][layer_idx] = {}
        cache_dic["attn_map"][-1]["single_stream"][layer_idx]["total"] = {}

        cache_dic["k-norm"][-1]["single_stream"][layer_idx] = {}
        cache_dic["k-norm"][-1]["single_stream"][layer_idx]["total"] = {}

        cache_dic["v-norm"][-1]["single_stream"][layer_idx] = {}
        cache_dic["v-norm"][-1]["single_stream"][layer_idx]["total"] = {}
    ...
    if max_order is not None:
        # max_order only makes sense for Taylor-style predictors.
        # For non-Taylor modes (e.g., ToCa/original/Delta), keep it at 0 to avoid
        # accidentally enabling higher-order predictions and breaking cache logic.
        if cache_dic.get("taylor_cache", False):
