...
configs = {
    "flux-dev": ModelSpec(
        repo_id="model-provider/FLUX.1-dev",
    ...
    "flux-schnell": ModelSpec(
        repo_id="model-provider/FLUX.1-schnell",
    ...
    "flux-dev-canny": ModelSpec(
        repo_id="model-provider/FLUX.1-Canny-dev",
    ...
    "flux-dev-depth": ModelSpec(
        repo_id="model-provider/FLUX.1-Depth-dev",
    ...
    "flux-dev-fill": ModelSpec(
        repo_id="model-provider/FLUX.1-Fill-dev",
...
def load_flow_model(
    ...
    if (
        ...
        local_path = _resolve_local_weight_path(name, configs[name].repo_flow)
        if local_path:
            _maybe_set_offline_env()
...
def _resolve_local_model_path(candidates: list[str]) -> str | None:
...
def _maybe_set_offline_env():
    os.environ.setdefault("HF_HUB_OFFLINE", "1")
    os.environ.setdefault("HF_DATASETS_OFFLINE", "1")
    os.environ.setdefault("TRANSFORMERS_OFFLINE", "1")


def _resolve_local_weight_path(name: str, filename: str) -> str | None:
    project_root = os.getenv("PROJECT_ROOT", os.getcwd())
    if "schnell" in name:
        base_dirs = ["FLUX.1-schnell", "flux-schnell", "flux.schnell", "schnell"]
    else:
        base_dirs = ["FLUX.1-dev", "flux-dev", "flux.dev", "dev"]
    for base in base_dirs:
        candidate = os.path.join(project_root, "models", base, filename)
...
def load_t5(device: str | torch.device = "cuda", max_length: int = 512) -> HFEmbedder:
    ...
    # Prefer local override via env var; fallback to public repo id
    t5_source = os.getenv("T5_DIR", "")
    if not t5_source:
        project_root = os.getenv("PROJECT_ROOT", os.getcwd())
        candidates = [
            os.path.join(project_root, "models", "t5-v1_1-xxl"),
            os.path.join(project_root, "models", "google", "t5-v1_1-xxl"),
        ]
        local_path = _resolve_local_model_path(candidates)
        if local_path:
            _maybe_set_offline_env()
            t5_source = local_path
    if not t5_source:
...
def load_clip(device: str | torch.device = "cuda") -> HFEmbedder:
    ...
    if not clip_source:
        project_root = os.getenv("PROJECT_ROOT", os.getcwd())
        candidates = [
            os.path.join(project_root, "models", "clip-vit-large-patch14"),
            os.path.join(project_root, "models", "clip-vit-large-patch14", "clip-vit-large-patch14"),
            os.path.join(project_root, "models", "openai", "clip-vit-large-patch14"),
        ]
        local_path = _resolve_local_model_path(candidates)
        if local_path:
            _maybe_set_offline_env()
            clip_source = local_path
    if not clip_source:
...
def load_ae(name: str, device: str | torch.device = "cuda", hf_download: bool = True) -> AutoEncoder:
    ...
    if (
        ...
        local_ae = _resolve_local_weight_path(name, configs[name].repo_ae)
        if local_ae:
            _maybe_set_offline_env()
            ckpt_path = local_ae
        else:
